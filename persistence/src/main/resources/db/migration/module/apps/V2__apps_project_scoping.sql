-- K-45 step 5: custom apps become project-scoped (typed container) — the mirror of
-- module/notes/V2 for the APPS COLLECTION container type.
...
ALTER TABLE t_custom_apps ADD COLUMN project_id UUID;
...
-- Strict model: every existing custom app joins the default APPS container, then the
-- column is locked NOT NULL. Custom app children (properties/records/views) cascade
-- through the app.
UPDATE t_custom_apps
SET project_id = (SELECT id FROM t_projects
                  WHERE project_type = 'APPS' AND is_default = true AND is_deleted = false)
WHERE project_id IS NULL;

ALTER TABLE t_custom_apps ALTER COLUMN project_id SET NOT NULL;

ALTER TABLE t_custom_apps ADD CONSTRAINT fk_custom_apps_project
    FOREIGN KEY (project_id) REFERENCES t_projects(id) ON DELETE CASCADE;
CREATE INDEX idx_custom_apps_project ON t_custom_apps(project_id);
