...
- `NoteRepository` (`JpaSpecificationExecutor` — `?q=` + category/pinned filters) · `NoteCategoryRepository` (`existsByName`/`existsByNameAndIdNot`, `JpaSpecificationExecutor`) — K-44 notes modülü, tenant şeması.
