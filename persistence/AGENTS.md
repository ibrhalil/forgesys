...
- **Role inheritance (Faz 4a):** `t_role_parents` (self-M2M on `t_roles`, created in the tenant `V1__baseline.sql` migration). Soft-deleted roles are filtered out of the parent traversal by `@SQLRestriction`, so orphan join rows left by a soft-deleted parent are harmless. `ON DELETE CASCADE` never fires (soft-delete is an UPDATE), consistent with the other join tables.
