...
- [x] **Güvenlik düzeltmeleri (2026-08-16/17, [RISK-36](docs/DECISIONS.md#risk-36)):** (1) **RbacSeeder startup privilege escalation kapandı** — eski kod her restart'ta rol'süz kullanıcıları all-permissions `Admin`'e atıyordu; Admin artık yalnızca provisioning'de explicit `assignAdminTo(user)` ile verilir (`RbacSeederTest` regresyonu). (2) Aktif `lockedUntil` penceresi refresh'te de bloklu (`CustomUserDetails.isEffectivelyNonLocked` — kilitli hesap refresh ile yeni access token basamaz). (3) `RedisRefreshTokenStore.revoke` rotasyon zincirini (`rotatedTo`) takip ediyor (logout↔rotate yarışı). **Bilinen açık P2'ler ÇÖZÜLDÜ (2026-08-23):** Redis kesintisi davranışı bilinçli — rate-limit + blacklist fail-open (connection exception yakalanır, 500 yok), `RedisRefreshTokenStore.rotate` → temiz 401 `Unknown`, session list/revoke → boş/false best-effort; yalnız `issue` fail-closed → `DataAccessException` 503 `service_unavailable` mapler (`GlobalExceptionHandler.handleDataAccess`, yeni ErrorCode). `InMemoryRefreshTokenStore.revoke` `rotatedTo` zincirini takip ediyor (Redis paritesi). Kalan (bilinçli): revoke zincir yürüyüşü Redis'te atomik değil (Lua değil) — çoklu-instance çift-logout yarışı, sınırlı zarar. Detay: [`docs/DECISIONS.md` RISK-36](docs/DECISIONS.md#risk-36).
