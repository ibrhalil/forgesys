...
- [ ] **Faz 3 — Rate limiting (defense-in-depth):** Bucket4j+Redis (app, tenant/IP/email — login/verify/refresh, özellikle unknown-email yolu) + Nginx `limit_req` (edge, K-33).
- [ ] **Faz 4 — RBAC model:** rol inheritance (`t_role_parents` + cycle guard + `resolveAuthorities` traversal) + ABAC service-layer sahiplik şablonu (Notes/Warehouse/Logistics için) + permission resource/action normalize.
