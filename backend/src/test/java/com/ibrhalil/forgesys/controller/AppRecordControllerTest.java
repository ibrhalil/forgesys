 ...
 * and covered by the gated {@code AppBuilderIT}.
 */
@SpringBootTest
@ActiveProfiles("test")
@Transactional
class AppRecordControllerTest extends AbstractRbacWebTest {
    ...
    void listRequiresAuthentication() throws Exception {
        mockMvc.perform(get("/api/v1/apps/" + app.getId() + "/records"))
    ...
    void listForbiddenWithoutReadPermission() throws Exception {
        mockMvc.perform(get("/api/v1/apps/" + app.getId() + "/records").cookie(auth("nop@tenant.test")))
    ...
    void createReturns201WithValues() throws Exception {
        perform(post("/api/v1/apps/" + app.getId() + "/records").contentType(JSON)
                        .content(recordBody("""
                                "%s":"Acme","%s":1500,"%s":"open\""""
                                .formatted(name.getId(), amount.getId(), stage.getId()))),
                "apps:record:write")
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.appId").value(app.getId().toString()))
    ...
    void createMissingRequiredPropertyReturns400() throws Exception {
        perform(post("/api/v1/apps/" + app.getId() + "/records").contentType(JSON)
                ...
                .andExpect(jsonPath("$.code").value("app_record_value_invalid"));
    }
    ...
    void createInUnknownAppReturns404() throws Exception {
        perform(post("/api/v1/apps/" + UUID.randomUUID() + "/records").contentType(JSON)
                        .content("""
                                {"values":{}}"""),
                "apps:record:write")
    ...
    void patchPartiallyUpdatesAndClears() throws Exception {
        AppRecord record = seedRecord(app.getId(), name, "Acme", amount, "1500");

        // Overwrite the name, keep the amount untouched (absent key).
        perform(patch("/api/v1/apps/" + app.getId() + "/records/" + record.getId()).contentType(JSON)
    ...
    void getReturnsRecordWithValues() throws Exception {
        ...
        perform(get("/api/v1/apps/" + app.getId() + "/records/" + record.getId()), "apps:record:read")
                .andExpect(status().isOk())
    ...
    void listReturnsRecordsPaged() throws Exception {
        ...
        perform(get("/api/v1/apps/" + app.getId() + "/records"), "apps:record:read")
                .andExpect(status().isOk())
    ...
    void recordIsScopedToItsApp() throws Exception {
        App other = seedApp("Inventory");
        AppRecord recordInOther = seedRecord(other.getId(), name, "Acme", null, null);

        perform(get("/api/v1/apps/" + app.getId() + "/records/" + recordInOther.getId()), "apps:record:read")
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("resource_not_found"));
    }
    ...
    void deleteReturns204() throws Exception {
        AppRecord record = seedRecord(app.getId(), name, "Acme", null, null);

        perform(delete("/api/v1/apps/" + app.getId() + "/records/" + record.getId()), "apps:record:delete")
    ...
    private AppProperty seedProperty(UUID appId, String propertyName, PropertyType type,
    ...
    private AppRecord seedRecord(UUID appId, AppProperty requiredName, String nameValue,
                                 AppProperty optionalAmount, String amountValue) {
