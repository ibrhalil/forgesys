...
@ActiveProfiles("test")
@Transactional
class ProjectCustomAppControllerTest extends AbstractRbacWebTest {
    ...
    void listRequiresAuthentication() throws Exception {
        mockMvc.perform(get("/api/v1/projects/" + appsProject.getId() + "/custom-apps"))
    ...
    void listInUnknownProjectReturns404() throws Exception {
        mockMvc.perform(get("/api/v1/projects/" + UUID.randomUUID() + "/custom-apps")
                        .cookie(auth("reader@tenant.test", "apps:customapp:read")))
    ...
    void listInNonAppsProjectReturns409() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/projects/" + notes.getId() + "/custom-apps")
                        .cookie(auth("reader@tenant.test", "apps:customapp:read")))
    ...
    void listScopesToItsContainer() throws Exception {
        ...
        seedCustomApp(appsProject.getId(), "CRM");
        seedCustomApp(other.getId(), "Inventory");
    ...
    void createReturns201AndAnchorsToContainer() throws Exception {
        perform(post("/api/v1/projects/" + appsProject.getId() + "/custom-apps")
                        .contentType(JSON)
                        .content("""
                                {"name":"CRM","icon":"📊"}"""),
    ...
    void createInNonAppsProjectReturns409() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/projects/" + notes.getId() + "/custom-apps")
                        .contentType(JSON)
                        .cookie(auth("writer@tenant.test", "apps:customapp:write"))
                        .content("""
                                {"name":"CRM"}"""))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.code").value("project_type_mismatch"));
    ...
    void updateMovesAppBetweenAppsContainers() throws Exception {
        Project other = seedProject(ProjectType.APPS, "Other Apps");
        CustomApp app = seedCustomApp(appsProject.getId(), "CRM");

        mockMvc.perform(put("/api/v1/custom-apps/" + app.getId())
                        .contentType(JSON)
                        .cookie(auth("writer@tenant.test", "apps:customapp:write"))
                        .content("""
    ...
    private CustomApp seedCustomApp(UUID projectId, String name) {
