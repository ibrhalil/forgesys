...
/**
 * Native PostgreSQL JSONB search over {@code t_custom_app_record_values} (K-15): executes
 * clauses pre-validated by {@link CustomAppQueryValidator} — SQL is assembled exclusively
 ...
 * Rationale: docs/CODE_NOTES.md (backend/service → CustomAppRecordSearchExecutor).
 */
@Repository
public class CustomAppRecordSearchExecutor {

    private static final String VALUE_TABLE = "t_custom_app_record_values";

    @PersistenceContext
    private EntityManager entityManager;

    public Page<UUID> search(UUID customAppId, List<ValidatedFilter> filters, List<ValidatedSort> sorts,
                             Pageable pageable) {
        // Fragments appended in final-SQL order; the running ?N index matches the bind list.
        ParamSql where = new ParamSql();
        where.append("r.custom_app_id = ?", customAppId);
        ...
        String countSql = "SELECT count(*) FROM t_custom_app_records r WHERE " + whereSql;

        String listSql = "SELECT r.id FROM t_custom_app_records r WHERE " + whereSql
        ...
        List<UUID> ids = rows.stream().map(CustomAppRecordSearchExecutor::asUuid).toList();
        ...
        switch (filter.operator()) {
            case IS_EMPTY, IS_NOT_EMPTY -> {
                ...
                String exists = "EXISTS (SELECT 1 FROM " + VALUE_TABLE + " v"
                        + " WHERE v.record_id = r.id AND v.property_id = " + marker + ")";
                sql.fragments.add(filter.operator() == CustomAppValueOperator.IS_EMPTY ? "NOT " + exists : exists);
            }
            case EQ, NOT_EQ -> {
                ...
                sql.fragments.add(filter.operator() == CustomAppValueOperator.EQ ? exists : "NOT " + exists);
            }
            case CONTAINS -> {
                String propertyMarker = sql.next();
                String valueMarker = sql.next();
    ...
    private String sqlComparator(CustomAppValueOperator operator) {
