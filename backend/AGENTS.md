...
> **Deferred (Epic 2.5 rest / 2.6):** refresh tokens + logout (Redis blacklist — granular per-session revoke), login-history write. **Token revocation DONE (RISK-21, 2026-07-24):** `JwtAuthenticationFilter` reads `UserAccount.tokenInvalidBefore` (single-column projection `UserRepository.findTokenInvalidBefore`) per request and rejects tokens whose `iat` (floored to seconds) strictly predates it. Set by `UserService.changePassword` / `resetPassword` / `revokeTokens(userId)` (new) and `AuthController.logout` (calls `revokeTokens(principal.userId)`). Scope is **user-scoped** (all outstanding tokens for the user — multi-device logout); granular single-token revoke is Epic 2.6 (Redis access-token blacklist). Per-request cost is one small indexed `t_user_accounts` lookup (Redis cache Epic 2.6). Deferred: tenant içi user email doğrulama + password reset akış (entity field'ları hazır: `emailVerificationToken`/`passwordResetToken`). **Brute-force lockout ([RISK-22]) DONE (login-scoped):** `AuthService.login` counts `failedLoginAttempts`/`lockedUntil` (5/15dk → `auth_account_locked` 423). The locked account's outstanding tokens stay valid until TTL (RISK-22 does not stamp `tokenInvalidBefore`); immediate lockout-revoke is a deliberate follow-up if needed.
