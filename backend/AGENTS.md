...
> **Deferred (Epic 2.5 rest / 2.6):** refresh tokens + logout (Redis blacklist), token revocation (`tokenInvalidBefore` check in the filter — column exists), brute-force lockout (`failedLoginAttempts`/`lockedUntil` fields exist), tenant içi user email doğrulama + password reset akış (entity field'ları hazır: `emailVerificationToken`/`passwordResetToken`).
