...
components/              # cross-feature/shared components
    ...
    ui/                    #   design system: Badge, Button, CheckboxList, ConfirmDialog,
                            ...
                            #   multi-sort chains (`sorts` + additive onSortChange via Shift+click), auto-refresh
                            #   interval in the settings menu (`onRefresh`)),
...
lib/                     # api (fetch + 401 refresh + K-55 searchQueryGet/sq wire + apiDownload/saveBlob),
                            ...
                            # useListPageState (list-page scaffold: page/sort(s incl. multi-sort chains)/search
                            # + debounce + page-reset + syncUrl + applySearchQuery + currentQuery),
                            # searchQuery (K-55 URL-state codec: list state JSON → base64url `sq` param),
...
- **Server-side list search/sort/filter (K-49):** pages get the whole scaffold from `lib/useListPageState` (`{page, setPage, pageSize, setPageSize, sort, toggleSort, search, setSearch, searchFields, setSearchFields, filters, setFilters, q, listParams}` — debounced search + the page-reset contracts). `listParams` (`{page, size, sorts: [sort], q, qFields, filters}` — empty optional keys absent) is the ready-made query argument: `useX(listParams)`, or `useX({ ...listParams, extra })` for scoped legacy params (audit/notes). **URL state (K-55):** opt-in `syncUrl: true` reflects the committed state (filters/sort/page/search) into the URL as a single base64url `sq` param via `lib/searchQuery` — shareable view links; mount precedence URL > storageKey > defaults; typing replaces history, committed changes push; popstate re-hydrates; the blob JSON mirrors the backend `SearchRequest` (the future `GET ?sq=` wire contract shares the codec). Enable per-page (pilot: request-logs). `sorts` serializes to the same repeated `sort=field,dir` wire params as the raw `sort` string.
