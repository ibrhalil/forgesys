import type { CustomAppProperty, CustomAppRecord, PropertyType } from './types';
...
// Pure helpers around customApp record cell values (JSON scalars keyed by property id);
...
export function isInlineEditable(prop: CustomAppProperty): boolean {
  return INLINE_EDITABLE_TYPES.includes(prop.type);
}

/** Human-readable cell text for the TABLE renderer; '' for empty cells. */
export function cellDisplay(prop: CustomAppProperty, record: CustomAppRecord): string {
...
export function cellEditValue(prop: CustomAppProperty, record: CustomAppRecord): string {
  const value = record.values[prop.id];
  return value === undefined || value === null ? '' : String(value);
}

/** First TEXT property (position order) — the natural "primary field" of an customApp. */
export function firstTextProperty(properties: CustomAppProperty[]): CustomAppProperty | undefined {
...
export function recordTitle(
  ...
  titleProp: CustomAppProperty | undefined,
  resolve?: (prop: CustomAppProperty, record: CustomAppRecord) => string,
): string {
  if (titleProp) {
