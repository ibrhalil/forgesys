...
const OPERATORS_BY_TYPE: Record<PropertyType, readonly AppValueOperator[]> = {
...
export function allowedOperators(type: PropertyType): readonly AppValueOperator[] {
  return OPERATORS_BY_TYPE[type];
}

/** IS_EMPTY/IS_NOT_EMPTY take no value (the backend rejects a non-null one). */
export function operatorNeedsValue(op: AppValueOperator): boolean {
...
function valueEquals(prop: AppProperty, cell: string | number, filter: string | number): boolean {
...
function compareValues(prop: AppProperty, cell: string | number, filter: string | number): number {
  if (prop.type === 'NUMBER') {
    const a = Number(cell);
...
export function matchFilter(prop: AppProperty, record: AppRecord, filter: AppValueFilter): boolean {
...
export function toWireFilter(filter: AppValueFilter): AppValueFilter {
