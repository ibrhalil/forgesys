...
export function RecordCalendar({
  ...
  const byDay = new Map<string, AppRecord[]>();
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button
          variant="secondary"
          size="sm"
          aria-label={t('apps.calendarPrev')}
        ...
        <Button
          variant="secondary"
          size="sm"
          aria-label={t('apps.calendarNext')}
          onClick={() => (mode === 'month' ? shiftMonth(1) : shiftWeek(1))}
        >
        ...
        <Button variant="ghost" size="sm" onClick={() => setAnchor(today)}>
          {t('apps.calendarToday')}
        </Button>
        <div role="group" aria-label={t('apps.calendarMode')} className="ml-auto flex items-center gap-0.5">
