...
export function RecordBoard({
  ...
  const [dragging, setDragging] = useState<AppRecord | null>(null);
  ...
  const move = (record: AppRecord, value: string | null) => {
    const key = ['apps', app.id, 'records', VIEW_RECORDS_PARAMS];
    const prev = qc.getQueryData<PageResult<AppRecord>>(key);
    qc.setQueryData<PageResult<AppRecord>>(key, (cur) =>
      cur
        ? {
            ...cur,
  ...
  const cellValue = (record: AppRecord): string | null => {
