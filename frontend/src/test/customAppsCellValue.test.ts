...
import {
  ...
  recordTitle,
  shortenId,
} from '../features/custom-apps/cellValue';
import type { CustomAppProperty, CustomAppRecord } from '../features/custom-apps/types';
...
const prop = (over: Partial<CustomAppProperty> = {}): CustomAppProperty => ({
  id: 'p1',
  customAppId: 'a1',
  name: 'P',
  type: 'TEXT',
...
const record = (values: CustomAppRecord['values'] = {}): CustomAppRecord => ({
  id: 'r1',
  customAppId: 'a1',
  values,
...
describe('buildRecordPatch (PATCH partial-merge diff)', () => {
  ...
  const stored: CustomAppRecord = record({ 'p-text': 'Old', 'p-num': 5, 'p-user': 'u-1' });
