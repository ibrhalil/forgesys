...
const TARGET_DETAIL = {
  ...
  name: 'Customers',
  properties: [
    { id: 'p-name', appId: TARGET_APP_ID, name: 'Name', type: 'TEXT', config: null, required: false, position: 0 },
  ],
  views: [],
};
...
const TARGET_RECORDS = {
  data: [
    { id: 't-1', appId: TARGET_APP_ID, values: { 'p-name': 'Acme Ltd' }, createdDate: '2026-08-01T00:00:00Z', updatedAt: '', createdBy: 'u' },
    { id: 't-2', appId: TARGET_APP_ID, values: { 'p-name': 'Globex' }, createdDate: '2026-08-02T00:00:00Z', updatedAt: '', createdBy: 'u' },
  ],
  meta: { page: 0, pageSize: 100, totalElements: 2, totalPages: 1, hasNext: false, hasPrevious: false },
};

const RELATION_PROP: AppProperty = {
  id: 'p-cust',
  appId: '99999999-9999-9999-9999-999999999999',
  name: 'Customer',
  type: 'RELATION',
  config: { targetAppId: TARGET_APP_ID },
  required: false,
  position: 1,
};
...
describe('RelationPicker', () => {
  ...
  it('offers target-app records titled by their first TEXT property', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    ...
    // Target detail + records are fetched through the shared apps cache keys.
    await waitFor(() =>
      expect(calls.some((c) => c.url === `/api/v1/apps/${TARGET_APP_ID}/records` || c.url.includes(`/api/v1/apps/${TARGET_APP_ID}/records?`))).toBe(true),
    );
