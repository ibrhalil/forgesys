...
- **Mail/SMTP ([K-48](DECISIONS.md#k-48) + [K-53](DECISIONS.md#k-53)):** sender profil ile split — prod `SmtpMailSender` (`spring.mail.*` → `MAIL_HOST`/`MAIL_PORT`/`MAIL_USERNAME`/`MAIL_PASSWORD`; `MAIL_HOST` boşsa startup fail-fast), dev `LogMailSender` (log'a düşer), test `InMemoryMailSender`. **Opt-in `smtp` companion profili** ([K-53](DECISIONS.md#k-53)): `dev,smtp` → dev'de gerçek SMTP; `application-smtp.yaml` aynı `MAIL_*` env'lerinden map'ler, default'lar dev-compose Mailpit'i (localhost:1025, UI :8025 — `test`/`prod` ile stack edilmez). Ortak config `forgesys.mail.*`: `from`, `default-language` (tr default), `templates-dir` (classpath `mail/*.html` override — `infra/templates/mail/`). Şablonlar TR/EN: tenant-verify, email-verify, password-reset. Self-hosted VPS mail + app-as-subdomain (`BASE_DOMAIN`/`APP_BASE_URL`) ops: [`MAIL_RUNBOOK.md`](MAIL_RUNBOOK.md).
