...
- **Etki:** Liste wire kontratı tek kodcit (base64url `sq`); POST /search kalıcı; düz-param GET'ler bookmark uyumu için kalır (FE kullanmaz). **Açık takipler:** sticky header (kapsayıcı yeniden yapısı), bulk BE endpoint'leri (users pilotı), saved views DB sürümü (tenant V6 migration onayı ister), platform service-accounts ölü arama kutusu, GET `findAll`/`search` servis dallanmasının birleştirilmesi.
